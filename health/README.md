...
- 输出文件：
  ...
  - `health/sleep_sessions.csv`：睡眠片段（20,505 条，包含开始/结束、时长小时、原始分类值、来源）
...
- 睡眠：`HKCategoryTypeIdentifierSleepAnalysis`，保留原始 `value`，计算 (end-start) 的小时数。

### 复现
```bash
python health/parse_export.py
# 生成 health/steps_daily.csv, energy_daily.csv, sleep_sessions.csv
