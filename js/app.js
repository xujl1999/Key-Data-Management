...
let activeTimeRange = "3M"; // 1M, 3M, 6M, 1Y
...
const initChartControls = () => {
  ...
  controls.addEventListener("click", (e) => {
    if (e.target.classList.contains("chart-btn")) {
      ...
      // 重新渲染
      if (currentMetricLabel) {
        renderTrendChart(currentMetricLabel);
      }
    }
  });
};
...
const renderTrendChart = async (metricLabel) => {
  ...
  try {
    ...
    // 根据时间范围筛选数据
    let days = 90;
    if (activeTimeRange === "1M") days = 30;
    else if (activeTimeRange === "6M") days = 180;
    else if (activeTimeRange === "1Y") days = 365;

    const startTime = lastTime - days * 86400000;
    const filteredSeries = series.filter((p) => p.date.getTime() >= startTime);

    // 计算趋势线（始终计算所有数据的移动平均，然后截取对应时间段，避免开头出现空缺）
    const fullTrend = computeRollingAvg(series, 7);
    const filteredTrend = fullTrend.filter((p) => p.date.getTime() >= startTime);
    ...
    const extent = calcExtent(filteredSeries, expandedGuideLines, isTime);
    ...
    const barData = filteredSeries.map((point) => ({
      value: [point.date.getTime(), point.value],
    }));

    const lineData = filteredTrend.map((point) => ({
      value: [point.date.getTime(), point.value],
    }));
    ...
    const option = {
      ...
      tooltip: {
        ...
        formatter: (params) => {
          ...
          let res = `<div>${dateLabel}</div>`;
          items.forEach(item => {
            const val = formatChartValue(item.value[1], isTime);
            const label = item.seriesName === "趋势" ? "7日均线" : "每日数值";
      ...
      series: [
        {
          ...
          lineStyle: {
            width: 1,
            color: "rgba(148, 163, 184, 0.3)" // 更加低调的蓝灰色
          },
          itemStyle: {
            color: "#94a3b8", // 点的颜色也调暗
            borderWidth: 0
          },
          markPoint: {
            symbol: 'circle',
            symbolSize: 10, // 小圆点作为锚点
            label: {
              show: true,
              backgroundColor: '#22d3ee', // 亮青色背景
              color: '#0f172a', // 深色文字
              borderRadius: 16, // 完全圆润
              padding: [6, 12],
              fontSize: 12,
              fontWeight: 600,
              formatter: (p) => {
                const name = p.data.type === 'max' ? 'Max' : 'Min';
                return `${name}: ${formatChartValue(p.value, isTime)}`;
              },
              shadowBlur: 10,
            ...
            data: [
              ...
              { type: 'min', name: '最小值', symbolOffset: [0, 20], label: { offset: [0, 10] } } // 最小值向下偏移
            ]
          },
